...
def mainloop():
    ...
    rthread = threading.Thread(target=readThread4socket, args=(client,))
    rthread.start()
...
def set_config():
    parser = argparse.ArgumentParser(
        prog="repeater2test half-duplex",
        description="testing program for uart2ethernet with oneway tx",
    )

    parser.add_argument("--arg1", "-p", "--port", type=str, help="uart port name [COMx,]", default="COM4")
    parser.add_argument("--arg2", "-s", "--sport", type=int, help="socket port [3200x,]", default=32000)
    parser.add_argument("--arg3", "-b", "--baudrate", type=int, help="baudrate [9600,...,921600]", default=921600)
